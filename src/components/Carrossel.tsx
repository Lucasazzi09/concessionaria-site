...
export default function Carrossel({ midias }: CarrosselProps) {
  ...
  useEffect(() => {
    if (midias.length <= 1) return; // Não precisa de timer se só tem 1 imagem
  ...
  // Se não tiver mídias, mostra fallback
  if (midias.length === 0) {
    return (
  ...
  return (
    <section style={styles.carrossel}>
      ...
      {/* Botões de navegação (aparecem só se tiver mais de uma imagem) */}
      {midias.length > 1 && (
        <>
